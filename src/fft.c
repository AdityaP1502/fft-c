...
bins deep_copy_bins_complex(bins xn, int length, int pad_length)
{
  // padded xn with zero
...
bins copy_bins_complex(bins xn, int length, int pad_length)
{
  // padded xn with zero
  bins padded_xn = malloc((length + pad_length) * sizeof(complex_number*));
...
void clear_pad(bins bin, int length, int pad_length)
{
  for (int i = length; i < length + pad_length; i++)
  {
    free(bin[i]);
...
int nearest_power_of_2(int length)
{
  ...
  {
    power_of_2 = power_of_2 << 1; // multiply by 2
  }

  return power_of_2;
}

bins convert_real_to_complex(double* xn, int length)
{
...
double* convert_complex_to_real(bins xn, int length)
{
