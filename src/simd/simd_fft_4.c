...
{  
    ...
    fft_br4_f_0(c_arr, j0, ql, hl);
    fft_br4_f_N_over_8(c_arr, w, j0 + eighthl, ql, hl, eighthl);
    fft_br4_f_N_over_16(c_arr, w, j0 + sixtenthl, ql, hl, sixtenthl);
    fft_br4_f_3N_over_16(c_arr, w, j0 + (3 * sixtenthl), ql, hl, 3 * sixtenthl);    
}

static void subproblem_routine_optimized_b(complex* c_arr, complex* w, int ql, int hl, int j0)
{  
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            // do butterfly with double complex
            fft_simd_br4_b(c_arr, w, start, k, ql, hl);
            start += 2;
            k += 2;
        }

        // normal butterfly
        fft_br4_b(c_arr, w, start, k, ql, hl);
        start += 2;
        k += 2;
    }

    // deal with special cases
    fft_br4_b_0(c_arr, j0, ql, hl);
    fft_br4_b_N_over_8(c_arr, w, j0 + eighthl, ql, hl, eighthl);
    fft_br4_b_N_over_16(c_arr, w, j0 + sixtenthl, ql, hl, sixtenthl);
    fft_br4_b_3N_over_16(c_arr, w, j0 + (3 * sixtenthl), ql, hl, 3 * sixtenthl);    
}
...
static void solve_N_4_ifft_r4(complex* c_arr, complex* w, int n_problems)
{
    int j0 = 0;
    int ql = 1;
    int hl = 2;
...
static void solve_N_16_fft_r4(complex* c_arr, complex* w, int n_problems)
{
    ...
    for (int i = 0; i < n_problems; i++)
    {
        ...
        fft_br4_f_N_over_8(c_arr, w, j0 + 2, ql, hl, 2);
        fft_br4_f_3N_over_16(c_arr, w, j0 + 3, ql, hl, 3);
        j0 += 16;
    }
}

static void solve_N_16_ifft_r4(complex* c_arr, complex* w, int n_problems)
{
    int j0 = 0;
    int ql = 4;
    int hl = 8;
    ...
    {
        fft_br4_b_0(c_arr, j0, ql, hl);
        fft_br4_b_N_over_16(c_arr, w, j0 + 1, ql, hl, 1);
        fft_br4_b_N_over_8(c_arr, w, j0 + 2, ql, hl, 2);
        fft_br4_b_3N_over_16(c_arr, w, j0 + 3, ql, hl, 3);
        j0 += 16;
    }
}
...
static void solve_fft_r4(complex* c_arr, complex* w, int fft_size)
{
    ...
    int l, hl, ql, j0, prev_ql, prev_hl;
    ...
    while (n_elements > 16)
    {
        ...
        for (int j = 0; j < ql; j++)
        {
            w->real[ql + j] = w->real[prev_ql + 4 * j];
            w->imag[ql + j] = w->imag[prev_ql + 4 * j];

            w->real[hl + j] = w->real[prev_hl + 4 * j];
            w->imag[hl + j] = w->imag[prev_hl + 4 * j];
        }
    }
...
static void solve_ifft_r4(complex* c_arr, complex* w, int fft_size)
{
    // W memory structure
    // w[0....N/4 - 1] = wl
    // w[N/4....N/2-1] = w2l
    ...
    while (n_elements > 16)
    {
        ...
        for (int i = 0; i < n_problems; i++)
        {
            subproblem_routine_optimized_b(c_arr, w, ql, hl, j0);
            j0 += n_elements;
        }
    ...
    if (n_elements == 16)
    {
        solve_N_16_ifft_r4(c_arr, w, n_problems);
        n_problems *= 4;
    }

    solve_N_4_ifft_r4(c_arr, w, n_problems);
}
...
{
    solve_ifft_r4(c_arr, W, fft_size);
